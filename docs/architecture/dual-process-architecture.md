...
- `[DRAW:要求]`：生成形象图（自拍/穿搭/"你现在在干嘛"）。
  - 用途：用户想看她的样子时触发生图。要求里写场景、动作、表情、构图，不能带方括号。
  - 行为：**旁路通路**，不需要也不影响 `[NEED_BACKEND]`，不进后脑工具循环。文字先发，
    图片在后台生成完（`asyncio.create_task`）追加发送。单轮只取第一个标记；
    per-runtime_key 只保留最新一张，新请求会取消上一张。
  - 前置：`draw` + `draw_desc` 两个模型别名都配置；未配置时该说明不注入前脑提示。
  - 只在**主对话轮**触发；轮询审查轮与主动消息轮只剥离标记不生图。
...
- `"等我拍一张！[DRAW:自拍,书桌前,微笑]"` → 文字立即发出，图片随后追发
...
> ⚠️ `core/routing.py` 有**两个**前脑解析器：`parse_front_brain_response()`（主对话）与
> `parse_front_brain_review()`（轮询审查）。新增任何标记都必须在两处都剥离，否则审查轮
> 会把标记当字面文本发给用户。`sanitize_adapter_output_text()` 里还有一层兜底剥离。
