...
从版本 1.x 开始，N.O.R.A. Core 采用**独立工作区**设计，将运行时数据与项目代码完全隔离：

```
项目代码 (N.O.R.A.Core/)
  ├── brain/          # 核心逻辑
  ├── platforms/      # 平台适配器
  ├── config.yml      # 配置文件（禁止 AI 读取）
  └── .env            # 环境变量（禁止 AI 读取）

工作区 (~/.nora/workspace/)  ← WORKSPACE_ROOT
  ├── skills/         # 技能脚本（AI 可读写）
...
- ✅ **代码与数据分离** - 项目目录保持整洁
- ✅ **便于备份** - 独立备份工作区即可保留所有技能和数据
- ✅ **安全隔离** - 敏感配置文件在项目目录，AI 工作在独立空间
- ✅ **灵活部署** - 不同环境可使用不同的工作区路径
