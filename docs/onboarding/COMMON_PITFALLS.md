...
- **别在"构造模型可见 history"时剥时间戳**（详见 5.17）。`strip_timestamp_markers` 现在只剩
  **输出侧**用途（模型回复发给用户前、去重兜底比较、轮询转述后脑结果）；构造 history
  统一用 `core.message_dedup.build_history_messages`，它**保留**时间戳前缀。
...
- 体检 SQL：`SELECT level, COUNT(*) FROM summaries GROUP BY level;`
  `level=1` 上百条且 `level=3` 为 0 = 归档长期没跑（见 5.14；新代码下下一条消息
  就会自动收敛，不会再产生 400）。
- 顺带：这类"实例 A 坏、实例 B 好，配置还一样"的问题，**先比数据量再比配置**。
  那次三个实例配置逐项相同，差别是摘要数 1 / 308 / 797。

## 5.14 归档从未执行过：阈值挂在会被自己排空的变量上

现象（2026-09-16 生产）：上一节的结构问题修好后 400 消失了，但**上下文依然巨大**
（单次请求 148533 input tokens）。`summaries` 表里 `level=1` 有 308 条、`level=3` **0 条**
—— 归档（二级压缩）**从来没有执行过**。日志里全库搜不到一条
`超过归档阈值` 或 `创建归档总结`。

根因是三个独立缺陷叠在一起，前两个各自都足以让归档永远不出结果：

**① 归档判据用错了变量（主因）。** `_check_and_compress()` 只算了一个 `count`
（`is_archived=0 AND is_pinned=0` 的消息数），拿它同时判压缩和归档。但压缩每次恰好
排掉 `compress_ratio` 条，`count` 于是被**它自己**压在 `compress_window` 附近
（生产实测稳定在 41~51），`archive_threshold(500)` 永远够不到。

> 判据要挂在**只增不减**的量上。归档的正确判据是"已被一级总结消化的消息总量"：
> `SELECT SUM(message_count) FROM summaries WHERE level=1`。
> 归档唯一还能触发的场景是"压缩持续失败"——而那恰恰是最不该再发 LLM 调用的时刻。

**② 那条 INSERT 的占位符和参数对不上。** `VALUES (?, ?, 3, ?, ...)` 只有 **9** 个 `?`，
却传了 **10** 个参数（`level` 多传了一个字面量 `3`），SQLite 抛
`ProgrammingError: Incorrect number of bindings supplied`。因为 ① 让它从未触发，
这个错误在线上一直没暴露 —— **只修 ① 的话，归档一触发就天天失败**。

**③ 归档会写第二条、读不到旧的那条。** 读取侧原本是 `ORDER BY timestamp DESC LIMIT 1`，
每轮归档各写一条的话，旧归档就成了永远读不到也删不掉的孤儿。归档要改成**滚动合并**
（吸收上一轮归档），删除旧行**必须按 id 删** —— 用
`WHERE platform=? AND chat_id=? AND level=3` 会把同一事务里刚插入的新归档一起删掉，
表现为"日志说归档成功、库里一条不剩"。

排查这类"某个后台任务从未生效"的通用手法：

- **先看有没有日志**。这一条能立刻区分"跑了但失败"和"根本没触发"。
- **看阈值挂的变量会不会被任务自身改变**。压缩/清理类任务尤其容易踩：被清理的对象
  同时又是触发条件。
- **直接调一次那个函数**（绕开上层 try/except 吞异常），看是真抛异常还是提前 return。
  ②就是这么做才暴露的。
- **重试队列是空的**通常意味着"从未触发"，而不是"失败了重试到 dead"。

已有库的自愈：修好后**不需要手工清库**。`SUM(message_count)` 立刻就超过阈值，
下一条消息触发归档，重复行随被吸收行一并删除。生产库副本实测
**153908 字符 → 6092 字符**（约 148K → 5.5K tokens）。

## 5.15 裸 create_task 的后台任务：同一批活被并发干了好几遍

现象（2026-09-16 生产，与 5.14 同源）：`summaries` 表里 308 条一级总结，只对应
**217 个不同 `(start_message_id, end_message_id)` 区间** —— 91 条是重复。同一区间
最多堆了 4 条，而且**正文各不相同**（651/501/644/719 字符），说明是 4 次独立的 LLM
调用，不是简单复制。重复内容占摘要总字符数的 **27%**，全部白烧 token。

根因：`add_message()` 每来一条消息就 `_launch_background(_check_and_compress)`，
而 `_launch_background` 是裸 `loop.create_task(coro)`——**没有互斥、也没有在途去重**。
突发时段（生产有过单日 422 条）几十个 worker 同时读到同一个 count，同时执行
`SELECT ... WHERE is_archived=0 ORDER BY timestamp ASC LIMIT compress_ratio`，
在任何 commit 之前取到的都是**同一批**最早的未归档消息，于是各自生成一条摘要、
各自 INSERT。

修法：`MessageHistory._acquire_compress_gate()/_release_compress_gate()`，
按 `(platform, chat_id)` 做在途互斥。闸门放在 `_perform_compression()` /
`_create_archive_summary()` 这一层（而不是只放在 `_check_and_compress`），因为
**重试 worker 会绕过 `_check_and_compress` 直接调这两个方法**，放上面会漏掉那条路径。

要点：

- **检查与置位之间不能有 `await`**，否则在单事件循环里也不是原子的。
- 拿不到闸门就**直接 return**，不要排队等待——这些任务本来就是"下次消息会再触发一次"
  的幂等检查，排队只会堆积任务。
- return 时**不要**调 `_mark_retry_success`：重试队列里那条记录必须留在 `pending`，
  等下一轮再跑。no-op 被当成成功出队就等于把任务弄丢了。
- 验证测试别只看结果。`test_concurrent_compression_does_not_duplicate_summaries`
  的断言要卡"LLM 调用次数 == 1"，否则把闸门拆了测试也照样过。
  本地实测：拆掉闸门 → 3 次调用 / 3 条摘要；装回去 → 1 次 / 1 条。

## 5.16 按 id 区间标记"已归档"：别依赖时间戳单调

`_perform_compression()` 原来用 `UPDATE messages SET is_archived = 1
WHERE id >= start_id AND id <= end_id` 标记已归档。

它**在"id 顺序 == 时间顺序"时是等价的**（区间内不可能有未归档却没被选中的消息——
选的就是最早的 N 条未归档），生产库实测也确实 0 次时间倒挂。所以这**不是**一个已经
发作过的 bug——看到 "区间 71~154 但 message_count 只有 10" 别急着下结论，那可能是
几十条摘要**累积**把这段标满的，不是一条吞掉的。

但那个等价性依赖一条**没写在任何地方**的隐含前提。一旦平台乱序投递、或带历史时间戳
回填，区间内没被总结的消息会被一并标成已归档，从此不再进上下文（静默丢记忆）。
现在改成按实际选中的 id 列表标记，不再依赖该前提；区间也改用选中集合的 `min/max id`
而不是"排序后首尾那条"。

> 教训：**先验证因果再下结论**。这次是查了 `LAG(timestamp) OVER (ORDER BY id)` 发现
> 倒挂次数为 0，才把"吞掉邻居"这个错误假设排掉的。反过来也一样：同一个现象
> （区间宽、条数少）有个更平凡的解释（多条摘要累积）时，先排除它。

## 5.17 时间戳：剥在"输出侧"是对的，剥在"模型输入侧"是错的

`strip_timestamp_markers` 曾经在**两条完全不同的链路**上被用：

| 链路 | 该不该剥 | 现状 |
|---|---|---|
| 模型回复发给用户前 / 去重兜底比较 / 轮询转述后脑结果 | **该剥** | 保留 |
| 构造模型可见 history（前脑、审查、打断判定、轮询） | **不该剥** | 已改为 `build_history_messages`（保留） |

**为什么输入侧不该剥**：`add_message` 写入的 `[<时间>] <原文>` 前缀是模型判断
"这句话是多久以前说的"的**唯一锚点**。全剥掉之后，整个 prompt 里只剩 system 尾部
一个"当前时间"，模型既不知道对话间隔，也算不出时间跨度——这是"时间观念乱"的正面原因。
注意**后脑从来没剥过**（直接 `msg["content"]` 进 temp_history），所以踩坑时容易
发现"只有前脑对时间犯迷糊"，别因此去怀疑别的链路。

**为什么保留它不破坏缓存**（这是最容易想反的一点）：该前缀是**入库那一刻冻结**的，
之后每轮读出来字节完全一致，落在可缓存的稳定前缀里，**零成本**。
真正每轮都变的是"当前时间"——它必须落在**最后一条 user 消息**里（缓存按前缀匹配，
拼装顺序是 system → history → 当前消息；放在 system 里会每轮作废整个前缀，
而放在最后一条消息里，这条消息本来每轮就是新的，边际成本为零）。

> 别把它改成"读取时现算当前时间拼进每条历史"。那样每轮 history 都变、缓存前缀全废。
> `tests/test_current_message_dedup_e2e.py::test_front_brain_history_timestamp_is_frozen_not_per_turn`
> 就是锁这一条的。

**判断新代码该走哪条**：问"这段文本是**要发给用户/给模型转述**的，还是**要给模型当历史读**的"。
前者用 `strip_timestamp_markers`，后者用 `core.message_dedup.build_history_messages`。

**配套的"一处现在"原则**：历史前缀 = 过去（每条消息各自的发生时刻），
当前时间 = 现在（每轮现算，只注入当轮最后一条 user 消息）。任何时刻整个 prompt
里只应有一处"现在"，否则模型分不清哪个是真的。

### ⚠️ `[系统备注]` / `[内部备注]` 块后面必须留空行

`_INTERNAL_NOTE_BLOCK_PATTERN` / `_SYSTEM_NOTE_BLOCK_PATTERN` 的终止符是
`\n\s*\n` **或字符串结尾**。所以有两种写法都是对的：
块在**最末尾**（靠 `$` 终止），或块后**跟一个空行**（靠 `\n\s*\n` 终止）。

危险的是第三种：**块后面还拼了别的块、且块尾没空行**——剥除会把后续内容一起吞掉。
前脑的图片加载失败提示曾经就是这种写法，图片加载失败 + 懒加载词库命中同时发生时，
词库块被整块吞掉。往 user_prompt 追加块时，**先确认已有块的位置与结尾空行**。
