...
- 在调用 `create_new_skill` 前，必须先阅读 `workspace/skills/skill_creator/` 下的文档（至少本 `SKILL.md`），确保命名、参数、流程符合规范。
...
3.  **完善 SKILL.md:** 在脚手架生成的 `workspace/skills/<skill_name>/SKILL.md` 基础上，用 `write_file`/`edit_file` 按下方模板补全参数、示例和约束。
...
1.  <b>编写可工作脚本:</b> 在主人批准 `SKILL.md` 后，开始编写实现核心功能的 Python 脚本 (e.g., `workspace/skills/<skill_name>/script.py`)。脚本必须具备实际作用，而非仅仅是一个框架或示例。
2.  **遵循 I/O 契约:**
...
- 配置注入：若存在 `workspace/skills/<skill_name>/config.json`，会以环境变量形式注入；不要去读根目录的 `config.yml` / `.env`。
- 密钥/配置储存：请统一储存在 `workspace/skills/<skill_name>/config.json` 内，执行代码时会以环境变量形式注入。
