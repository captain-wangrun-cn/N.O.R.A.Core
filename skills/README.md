...
## 目录与入口
- 每个技能位于 `skills/<skill_name>/`，至少包含 `SKILL.md` 与 `main.py`。
- `SKILL.md` 需带 YAML Frontmatter，至少包含 `name`、`description`，可选 `parameters`（供运行时生成调用 schema）。
- 入口脚本统一放在 `skills/<skill_name>/main.py`，用 `python3` 直接运行，避免再加子目录。

## 运行时环境与路径
- 框架通过工具 `execute_skill` 调用脚本，并注入这些环境变量：
  - `WORKSPACE_ROOT`：仓库根路径
  - `SKILLS_DIR`：技能目录（通常是 `<repo>/skills`）
  - `DOWNLOADS_DIR`：下载/临时文件目录
  - `CODE_ROOT`：源代码根路径（与 `WORKSPACE_ROOT` 一致）
...
## 配置与秘密
- 技能私有配置放在 `skills/<skill_name>/config.json`（JSON 对象）；运行时会自动注入为环境变量。
...
## 文档要求
- `SKILL.md` 需包含功能说明、参数、示例和注意事项，可参考 `skills/skill_creator/SKILL.md` 模板部分。
...
## 最小操作流程
1) 创建目录 `skills/<skill_name>/`，或使用工具 `create_new_skill` 生成骨架。
